...
bool gui::load()
{
  ...
  char argv1[] = "-platform";
  char argv2[] = "windows";
  char argv3[] = "-platformpluginpath";
  char pluginDir[512];

  //sprintf(pluginDir, "D:\\vcpkg\\installed\\x64-windows\\debug\\plugins");
  sprintf(pluginDir, "G:\\dev\\Qt\\5.12.5\\msvc2015_64\\plugins");
  char* argv[] = { argv0, argv1, argv2, argv3, pluginDir, "\0" };
  ...
  for (int i = 0; i < g_argc; i++)
  {
	...
	g_argv[i] = new char[strlen(argv[i]) + 1];
	snprintf(g_argv[i], strlen(argv[i]) + 1, argv[i]);
  }
